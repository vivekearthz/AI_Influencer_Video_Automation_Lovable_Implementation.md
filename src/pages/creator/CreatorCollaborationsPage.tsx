...
export function CreatorCollaborationsPage() {
  ...
  return (
    <div>
      <PageHeader title="Collaborations" description="Every brand collaboration, past and present." />
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            <p className="p-6 text-sm text-muted-foreground">Loading…</p>
          ) : !collaborations?.length ? (
            <p className="p-8 text-center text-sm text-muted-foreground">No collaborations yet.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
